...
function calculateScore(votes: number, addedAt: number): number {
  const MAX_TIMESTAMP = 10000000000000; // Year 2286
  return (votes * 1_000_000) + (MAX_TIMESTAMP - addedAt);
}
...
export async function addSong(
  ...
  await rClient.zAdd(
    ...
    { score, value: JSON.stringify(song) }
  );
...
export async function getQueue(roomId: string): Promise<Song[]> {
  const songs = await rClient.zRangeWithScores(
    REDIS_KEYS.ROOM_QUEUE(roomId),
    0,
    -1,
    { REV: true }
  ...
  return songs.map(item => JSON.parse(item.value) as Song);
}
...
export async function voteSong(
  ...
  if (song.votedBy.includes(userId)) {
    throw new Error("User already voted on this song");
  }
  ...
  await rClient.zAdd(
    REDIS_KEYS.ROOM_QUEUE(roomId),
    { score: newScore, value: JSON.stringify(song) }
  );
...
export async function isQueueEmpty(roomId: string): Promise<boolean> {
  return (await rClient.zCard(REDIS_KEYS.ROOM_QUEUE(roomId))) === 0;
}

export async function getQueueLength(roomId: string): Promise<number> {
  return rClient.zCard(REDIS_KEYS.ROOM_QUEUE(roomId));
}
